...
class ResponseCache:
    ...
    def similarity(a: str, b: str) -> float:
        ...
        def tokenize(s: str) -> list[str]:
            lower = s.lower()
            words = lower.split()
            ngrams = [lower[i:i + 3] for i in range(len(lower) - 2)]
            return words + ngrams
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        for rkey in self._redis.scan_iter(f"{self.prefix}*"):
            cached_query = self._redis.hget(rkey, "query")
            ...
            if score > best_score:
                ...
                best_response = self._redis.hget(rkey, "response")
